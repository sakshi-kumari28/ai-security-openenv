...
# ── Required env vars ─────────────────────────────────────────────────────────
API_KEY      = os.environ["API_KEY"]           # FIXED: use injected key only, no fallback
API_BASE_URL = os.environ["API_BASE_URL"]      # FIXED: use injected base URL only, no fallback
MODEL_NAME   = os.getenv("MODEL_NAME", "Qwen/Qwen2.5-72B-Instruct")
TASK_NAME    = os.getenv("TASK_NAME", "data_leakage_prevention")
BENCHMARK    = "ai-security-openenv"
MAX_STEPS    = 1

client = OpenAI(api_key=API_KEY, base_url=API_BASE_URL)  # FIXED: no dummy-key fallback
...
class LLMAgentAdapter:
    ...
    def decide(self, state: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def run_task_with_logging(task_name: str = TASK_NAME) -> float:
    ...
    Uses LLMAgentAdapter so all decisions go through the LiteLLM proxy.
    Returns the final score in [0, 1].
    """
    log_start(task_name, BENCHMARK, MODEL_NAME)

    env      = AiSecurityEnv()
    llm      = LLMAgentAdapter()        # FIXED: use LLM agent so API calls are made
    fallback = SecurityAgentBaseline()  # fallback if LLM returns None
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        try:
            # FIXED: always call LLM first (this is what registers with the proxy)
            action = llm.decide(state)
            if action is None:
                action = fallback.decide(state)  # fallback if LLM fails
